...
void projectM::projectM_resetTextures()
{
 ...
 projectM::projectM ( std::string config_file, std::string presetUrl, int flags) :
		beatDetect ( 0 ), renderer ( 0 ),  _pcm(0), m_presetPos(0), m_flags(flags)
{
	readConfig ( config_file, presetUrl );
...
void projectM::readConfig (const std::string & configFile, const std::string &presetUrl)
{
    ...
    _settings.presetURL = presetUrl;
    /*
    #ifdef __ST_OS_ANDROID__
	_settings.presetURL = config.read<string> ( "Preset Path", "/sdcard/test/preset" );
	#endif
