...
void Renderer::Pass2(const Pipeline &pipeline, const PipelineContext &pipelineContext)
{
	...
	glLineWidth(this->renderTarget->texsize < 512 ? 1.0f : (float)(this->renderTarget->texsize / 512));
...
void Renderer::Interpolation(const Pipeline &pipeline)
{
	...
	glEnableClientState(GL_TEXTURE_COORD_ARRAY);
	glDisableClientState(GL_COLOR_ARRAY);
	glTexCoordPointer(2, GL_FLOAT, 5*sizeof(GLfloat), p); 
	glVertexPointer(3, GL_FLOAT, 5*sizeof(GLfloat), p + 2);
#endif
