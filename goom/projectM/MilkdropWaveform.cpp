...
void MilkdropWaveform::Draw(RenderContext &context)
{
	...
	glMatrixMode( GL_MODELVIEW );
		...
		//Thick wave drawing
		if (thick==1)  glLineWidth( (context.texsize < 512 ) ? 2.0f : (float)(2*context.texsize/512));
...
void MilkdropWaveform::WaveformMath(RenderContext &context)
{
	...
	switch(mode)
	{
		...
		case Circle:
		  {
	       ...
	       float last_value = context.beatDetect->pcm->pcmdataR[samples-1]+context.beatDetect->pcm->pcmdataL[samples-1];
			...
			for ( int i=0;i<samples;i++)
			{
			  ...
			  r=(0.5f + 0.4f*.12f*value*scale + mystery)*.5f;
			  theta=i*inv_nverts_minus_one*6.28f + context.time*0.2f;

			  wavearray[i][0]=(r*cos(theta)*(context.aspectCorrect? context.aspectRatio : 1.0f)+x);
		...
		case RadialBlob://circularly moving waveform
			...
			{
				theta=context.beatDetect->pcm->pcmdataL[i+32]*0.06f*scale * 1.57f + context.time*2.3f;
				r=(0.53f + 0.43f*context.beatDetect->pcm->pcmdataR[i]*0.12f*scale+ mystery)*.5f;
		...
		case Blob2://EXPERIMENTAL
			...
			for ( int i=0;i<512-32;i++)
			{
				wavearray[i][0]=(context.beatDetect->pcm->pcmdataR[i]*scale*0.5f*(context.aspectCorrect ? context.aspectRatio : 1.0f) + x);
				wavearray[i][1]=(context.beatDetect->pcm->pcmdataL[i+32]*scale*0.5f + temp_y);
			}
		...
		case Blob3://EXPERIMENTAL
			...
			for ( int i=0;i<512-32;i++)
			{
				wavearray[i][0]=(context.beatDetect->pcm->pcmdataR[i] * scale*0.5f + x);
				wavearray[i][1]=( (context.beatDetect->pcm->pcmdataL[i+32]*scale*0.5f + temp_y));
			}
		...
		case DerivativeLine://single x-axis derivative waveform
		{
			...
			for (int i=0; i<512-32; i++)
			{
				...
				yy[i] =0.4f* context.beatDetect->pcm->pcmdataL[i]*0.47f*scale + temp_y;
				xx[i] += 0.4f*context.beatDetect->pcm->pcmdataR[i]*0.44f*scale;
		...
		case Line://single waveform
			...
			for ( int i=0;i<  samples;i++)
			{
				...
				wavearray[i][1]=context.beatDetect->pcm->pcmdataR[i]*.04f*scale+wave_x_temp;
		...
		case DoubleLine://dual waveforms
			...
			{
				wavearray[i][0]=i/((float)  samples);
				wavearray[i][1]= context.beatDetect->pcm->pcmdataL[i]*.04f*scale+(wave_y_temp+y_adj);
			}

			for ( int i=0;i<samples;i++)
			{
				wavearray2[i][0]=i/((float)  samples);
				wavearray2[i][1]=context.beatDetect->pcm->pcmdataR[i]*.04f*scale+(wave_y_temp-y_adj);
			}
