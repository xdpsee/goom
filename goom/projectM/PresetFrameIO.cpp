...
void PresetInputs::Initialize ( int gx, int gy )
{
	...
	for ( x=0;x<gx;x++ )
	{
		for ( y=0;y<gy;y++ )
		{
			...
			this->origrad[x][y]=(float)hypot ( ( this->origx[x][y]- 0.5f ) * 2, ( this->origy[x][y] - 0.5f) *2 ) * 0.7071067f;
			this->origtheta[x][y]=atan2 ( ( ( this->origy[x][y] - 0.5f) * 2), (( this->origx[x][y] - 0.5f ) *2 ) );
		}
	}
	...
	for (x = 0; x < gx; x++)
	{
		...
		for (x = 0; x < gx; x++)
		{
			for (int y = 0; y < gy; y++)
			{
				...
				rad_mesh[x][y]=(float)hypot ( ( origx-.5f ) *2, ( origy-.5f ) *2 ) * .7071067f;
				orig_x[x][y] = (origx - .5f) * 2;
				orig_y[x][y] = (origy - .5f) * 2;
			}
		}
}
