...
public:
	...
	inline static void Zoom(Point_M &p, const PerPixelContext &context, float zoom, float zoomExponent)
	{
	...
	inline static void Transform(Point_M &p, const PerPixelContext &context, float dx, float dy)
	{
		p.x -= dx;
		p.y -= dy;
	}

	inline static void Scale(Point_M &p, const PerPixelContext &context, float sy, float sx, float cx, float cy)
	{
		p.x = (p.x - cx)/sx + cx;
		p.y = (p.y - cy)/sy + cy;
	}

	inline static void Rotate(Point_M &p, const PerPixelContext &context, float angle, float cx, float cy)
	{
		float u2 = p.x - cx;
		float v2 = p.y - cy;
	...
	inline static void Warp(Point_M &p, const PerPixelContext &context, float time, float speed, float scale, float warp)
	{
