...
PerPixelMesh::PerPixelMesh(int width, int height) : width(width), height(height), size (width * height),
		p(width * height, Point_M(0,0)),
		p_original(width * height, Point_M(0,0)),
		...
		for (int j=0;j<height;j++)
			for(int i=0;i<width;i++)
			{
				int index = j * width + i;
						...
						identity[index].rad=(float)hypot ( ( xval-.5f ) *2, ( yval-.5f ) *2 ) * .7071067f;
						identity[index].theta=atan2 ( ( yval-.5f ) *2 ,  ( xval-.5f ) *2  );
			}
		}
...
Point_M::Point_M(float x, float y)
	: x(x), y(y) {}
